...
@section("content")
    <form class="p-4 my-6 mx-4 bg-white rounded-lg shadow md:flex flex-col md:p-6 xl:p-8 dark:bg-gray-800" method="POST" action="{{route('users.detail.edit', ['id'=> $users->id])}}" enctype="multipart/form-data">
        @csrf
        @method("PATCH")
        <div class="w-full flex justify-center">
            <img id="users-avatar" src="{{asset('storage/'. $users->avatar)}}" class="h-44 rounded-lg" alt="">
        </div>
        <h3 class=" mt-5 flex items-center mb-4 text-lg font-semibold text-gray-900 dark:text-white">{{$users->name}}</h3>
        <div class="w-full flex flex-col gap-4">
            <div>
                <input type="hidden" name="name" value="{{ $users->name }}">
            </div>
            <div>
                <label for="avatar" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Gambar Webinar</label>
            ...
            <div>
                ...
                <input name="alamat" type="text" id="alamat" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$users->address}}" value="{{$users->address}}" required />
                @error('alamat')
                    <small class="text-red-500">{{ $message }}</small>
                @enderror
            </div>
            ...
            <div>
                ...
                <input name="role" type="text" id="role" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="{{$users->role}}" value="{{$users->role}}" readonly/>
                @error('role')
                    <small class="text-red-500">{{ $message }}</small>
                @enderror
            </div>
