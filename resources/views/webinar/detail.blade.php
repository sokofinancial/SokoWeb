...
@section("content")
    <form class="p-4 my-6 mx-4 bg-white rounded-lg shadow md:flex flex-col md:p-6 xl:p-8 dark:bg-gray-800" method="POST" action="{{route('webinar.detail.edit', ['slug'=> $webinar->slug])}}" enctype="multipart/form-data">
        @csrf
        @method("PATCH")
        <div class="w-full flex justify-center">
            <img id="webinar-image" src="{{asset('storage/'. $webinar->image)}}" class="h-44 rounded-lg" alt="">
        </div>
        <h3 class=" mt-5 flex items-center mb-4 text-lg font-semibold text-gray-900 dark:text-white">{{$webinar->title}}</h3>
